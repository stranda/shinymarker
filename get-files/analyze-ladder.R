...
analyze.standard <- function(fsadat,
                             ...
                             )
    {
        ...
        std.curve <- lm(actual.peaks~poly(peak.times,3)) #2nd order polynomial. Can choose whatever other function.  this gave really high r^2 on the couple I tried
        tmp$size <- predict(std.curve,newdata=data.frame(peak.times=tmp$time))
        list(data=tmp, model=std.curve)
    }
